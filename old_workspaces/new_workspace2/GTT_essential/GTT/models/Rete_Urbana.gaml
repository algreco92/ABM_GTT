...
global
{
	...
	// Per ogni fermata: matrice di attrazione di ogni utente, a seconda dell'orario, creata in base agli edifici
	// d'interesse a meno di 500 metri
...
//	bool salva <- true update:(time < 2000?((time mod 150 =0)? true:false):((time mod 800 =0)? true:false));
	...
	string salvataggio <- "./exports/"; // File in cui vengono salvati eventuali file durante l' experiment
	// doomsday è un riflesso che dopo ogni "reset_time" cicli ripristina tutti i dati del modello, in modo da poter
	// iniziare diversi esperimenti con un unico run.
init
	{
		...
		// 29782 è il numero di agenti da instanziare senza applicare scala.
		...
		// Creazione utenti
		// set_appeal_matrix lo faccio qui e non nell'init della specie perché alla fine dell'init cancello 
		if input_csv = true
		{
			time <- time_offset;
			file fileCSVpasseggeri <- file(salvataggio + "./input/passenger_population" + int(time_offset) + ".csv");
			matrix initiop <- matrix (fileCSVpasseggeri);
	
			file fileCSVSmith <- file(salvataggio + "./input/Smith_population" + int(time_offset) + ".csv");
			matrix initioS <- matrix (fileCSVSmith);
	
			file fileCSVGTT <- file(salvataggio + "./input/GTT_population" + int(time_offset) + ".csv");
			matrix initioG <- matrix (fileCSVGTT);

			file fileCSVwedge <- file(salvataggio + "./input/wedge_population" + int(time_offset) + ".csv");
			...
			loop row over: rows_list(initio2p) //where(each!=first(rows_list(initiop)))
			{
				create passenger
				{
...
//				int partenza2 <- int(edge_list_matrix[0, j]);
		...
		write "inizializzazione bus_stop";
		ask bus_stop
		{
		...
		// ridotto la rete ==> fermate scollegate ==> cancellazione fermate di troppo
...
species wedge
{
	...
	// Come per bus_stop popolazione_old è il numero di utenti presenti sulla fermata al turno precedente.
	int popolazione_old update: popolazione;
	// Agenti che viaggiano sull'edge. Posto a 0 ogni inizio turno dal reflex (così viene
	// posto a 0 dopo che popolazione_old copia il valore)
	int popolazione;
	bus_stop partenza;
	bus_stop arrivo;
	float penality <- 1.0 update: 1.0; // Ogni turno viene aggiornato ad 1 (sarà utilizzato dai controllori smith)
	list<float> avg_penality <- list_with(4, 1.0); // Diviso in 4 macrofasce (sarà utilizzato da GTT)
	float btw;
	...
	reflex update_variables
	{
		...
		// Serve a GTT per gestire i controlli
		...
		// Assegna ad ogni macrofascia uno score ad ogni edge, per stimare quanto sia interessante da controllare
		// 70 è il numero di giri in una settimana in una sola delle 4 macrofasce (280 giri / 4 macrofasce = 70).
...
//	float avg_risk_balance <- 0.0 update:0.0;//misura di quanto è cambiato il rischio medio nell'ultimo turno
	float avg_mobility;
	...
	int travel_mode update: 3; // 0-> viaggio alternativo; 1 -> viaggio senza ticket; 2-> viaggio con
							   //ticket; 3 -> il flip (mobility) ha dato croce)
	...
	reflex decisione_abbonamento when: (every(40) and titolo_viaggio < 40)
	{
		...
		// prossimo mese
		if (1200*self.avg_mobility*min([(self.avg_risk/max([1,self.num_viaggi]))*multa,ticket]) > abbonamento)
		{
			// Se si decide di fare l'abbonamento titolo viaggio ci mette un mese per azzerarsi
			titolo_viaggio <- 1200;
			GTT[0].utility <- GTT[0].utility + abbonamento; // GTT prende soldi
	...
	reflex movimento when: flip((1 + 0.4 * signum(titolo_viaggio)) * mobility[fascia_oraria, type])
	{
		...
		utility[2] <- 1 - exp(-length(percorso.edges) / ticket) + self.titolo_viaggio; //ViaggioConTicket
		...
		{ // 2 = voglio viaggiare e non voglio fare il portoghese, 0 = non ho il biglietto, ma me lo faccio
			...
			GTT[0].utility <- GTT[0].utility + ticket; // GTT prende i soldi del biglietto
		}
		if (travel_mode != 0)
		{ // Ora si parte!!
			ask list<wedge> (percorso.edges)
			{ //Ogni wedge aumenta di unità la popolazione se fa parte del percorso di un utente
...
species GTT
{ // GTT è l'azienda che controlla i movimenti dei controllori e tiene sotto controllo entrate e uscite (utility).
...
//	float risk_balance <- 0.0 update: risk_balance + sum(passenger collect each.avg_risk_balance);
	...
	{	// Ogni 280 giri (1 settimana), GTT paga i controllori, salva su csv i dati salienti,
		...
		[ // [0]tempo, [1]percentuale passeggeri viaggianti col biglietto, [2]Senza biglietto, [3]a piedi,
		  // [4]Utenti abbonati, [5]Soldi accumulati dall'azienda GTT, [6]Rischio totale.
			time,
	...
	{ // GTT is the god. It creates the Smith's.
		create Smith;
	}
...
species Smith
{
	...
	{ // Questa action assegna un nuovo target ad un controllore.
		...
		if random = false {targets[a] <- GTT[0].target_list[a][0];}
		...
		ask GTT[0]{do update_target_list(a);}
	}
	...
	{ // Qui il controllore fa le multe e viene aggiornato il rischio di quelli che sono passati sui suoi wedge.
		...
		loop edge over: percorso // Per tutti i wedge su cui passa il controllore in un giro (mezz'ora)...
		{
			...
			ask (int(48*edge.popolazione/edge.peso)) among passenger where (each.flag_movimento = true and edge in each.percorso.edges) // Peso va diviso per 2
			{ 
		...
		GTT[0].utility <- GTT[0].utility + multa*min([4,length(morosi)]);
...
//			species bus_stop aspect: base;
	    ...
	    display chart refresh_every: 1 																					//NEW
	    {
			...
			{
				data "Risk (magnified 10x)" value: 10*GTT[0].total_risk/n_passenger color: rgb("red");
...
experiment Breakeven type: batch repeat: 10 until: time = 20000 + time_offset keep_seed: false
{
	...
	parameter 'start_time' var: time_offset <- 146001.0;
	parameter 'input_csv' var: input_csv<- true;
	parameter 'percorso' var:salvataggio <- "./exports/break_even/";
