...
global
{
	...
	float reset_time <- 146000.0; // // // //Numero cicli di ogni simulazione
	graph grafo;
...
//	bool salva <- true update:(time < 2000?((time mod 150 =0)? true:false):((time mod 800 =0)? true:false));
	...
	{
		index <- index +1; // Cambia codice esperimento
		if (index = 10) // // // // Qui decidiamo il numero di esperimenti da fare in un unico run
		{
			write "halting";
    		do halt;
    	}
		// Ripristiniamo tutti i parametri di tutti gli agenti
		ask wedge
		{
			penality <- 1.0;
		...
		ask passenger
		{
			flag_abbonamento <- false; // senza abbonamento
			avg_risk <- 0.0; // Azzeramento rischio medio di incontrare un controllore
			num_viaggi <- 0.0; // azzeramento numero di viaggi
			type <- rnd(5); // Assegnazione random del tipo di utente
//			avg_risk_balance <- 0.0;
			viaggi <- 0.0 as_matrix ({ 5, 3 }); // Azzeramento viaggi
			risk <- 0.0 as_matrix ({ 5, 3 }); // Azzeramento rischio calcolato su una tratta
			titolo_viaggio<-0; // azzeramento tipo di comportamento utente
		}
		ask GTKIRA
		{
	...
	// Fine reflex doomsday


	init
	{
		//write fit_abbonamento;
		index<-0; // Codice numero esperimento a 0
		n_controllori <- 4; // numero controllori
	...
	reflex evaluation when: every(280)
	{
	 ...
	 //considerando un costo mensile di 2500€ a controllore, la GTT versa 625€ a controllore a settimana
		...
		[
		 ...
		 to:(salvataggio +"ticket_trend_"+ index +"_"+ Flip + ".csv") type: "csv";
		ask wedge
		{
			penality <- 1.0;
...

{
...

	{
		...
		ask wedge where (each in self.percorso) //penalizzazione wedge nel percorso
...
experiment Flipbatch type: batch repeat: 1 until: time =  72000 + time_offset keep_seed: true 
{
		...
		parameter 'Flip' var: Flip among: [0.0 , 1.0];
		parameter 'start_time' var: time_offset <- 146001.0;
		parameter 'input_csv' var: input_csv<- true;
}

experiment Stability type: batch repeat:1 until: time = 146000 keep_seed:true
{
