...
//			int partenza2 <- int(edge_list_matrix[0, j]);
		...
		write "wedge creati";
		grafo1 <- directed(as_edge_graph(wedge));
		ask wedge
		{
			do set_betweenness;
		}
...
species passenger skills: [moving]
{
	...
	matrix<list<float>> superrr <- matrix<list<float>>([0.0] as_matrix({5,3}));
	int type;
	...
	reflex decisione_abbonamento when: (every(40) and titolo_viaggio < 40) //**new
	{
		...
		if (1200*self.avg_mobility*((1-polinomio)*multa*x + polinomio*ticket) > abbonamento) //AGGIUNGERE GAUSSIANA
		{
			titolo_viaggio <- 1200;
...
//			write "inactive";
		...
		percorso <- path_between(grafo1, fermata.location, the_target.location);
...

	{
		...
		[
			...
			total_risk
			 ...
			 to:("percentuali_batch" + "_" + string(dimenticanza) + "_" + string(dimenticanza2) + ".csv") type: "csv";
...

{
	init
	{
		...
		{
			do acquire_new_targets(macro_fascia2);
		}
