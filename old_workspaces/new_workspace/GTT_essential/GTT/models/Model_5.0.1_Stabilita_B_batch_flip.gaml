...
//	bool salva <- true update:(time < 2000?((time mod 150 =0)? true:false):((time mod 800 =0)? true:false));
	...
	string salvataggio <- "./exports/13_11/";
...
species wedge
{
	...
	{
		saturation_index <- min([4, int(floor(48*4*(scala/100)) * (self.popolazione_old / self.peso))]); //vale per ridottissimo
...
//			write "inactive";
		...
		save[length(percorso.edges)] to:"./dati_lunghezze.csv" type:"csv";
...
//	reflex outprut when: (self.flag_movimento = true)
//		{
//			save [time,self.avg_risk/self.num_viaggi,(self.travel_mode = 2)? 1:0,self.avg_risk, self.num_viaggi_noweight] to:"./prova_theta.csv" type:"csv";
//		}
	aspect base
	{
		if (travel_mode != 3)
...
//	float risk_balance <- 0.0 update: risk_balance + sum(passenger collect each.avg_risk_balance);
	...
	reflex evaluation when: every(280)
	{
	 ...
	 //considerando un costo mensile di 2500€ a controllore, la GTT versa 625€ a controllore a settimana
		save
		[
			time, length(passenger where (each.flag_abbonamento = true)) * 100 / length(passenger),
		 ...
		 to:(salvataggio + "_ticket_trend"+  n_passenger +"_"+rnd_index + ".csv") type: "csv";
		loop j from: 0 to: length(target_list) - 1 //nuovi obiettivi settimanali
		{
			do update_target_list(j);
		}
		ask wedge
		{
			penality <- 1.0;
...
//		}
	...
	reflex verification
	{
		...
		{//popolazione/peso sono gli agenti che erano su quell'edge. Peso va diviso per 48
...
experiment flip_e_controllori type: batch repeat: 1 until: time = 58400 keep_seed: true
{
		parameter 'Flip' var: Flip among: [0.2 , 0.8];
		parameter "N_controllori" var: n_controllori among: [2 , 8];
}